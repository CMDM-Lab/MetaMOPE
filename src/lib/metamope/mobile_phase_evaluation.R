...
mobile_phases_list = input.arg[3]
mzxml_files_phases = input.arg[4]
...
mobile_phases <- unlist(mobile_phases_list)
...
for (i in 1:mobile_phase_n) {
  for (mode in c("pos", "neg")) {
    ...
    files <- unlist(mzXML_files_phases[i], pattern=paste0("_", mode, "_"))
